...
def group_nodes_by_host(nodes, roles='', env=''):
    ...
    guests = filter_nodes(nodes, virt_roles='guest', env=env)
    filtered_hosts = []
    ...
    for host in hosts:
        ...
        if has_role:
            print host['name'],host['virtualization']['guests']
            # Show only a host if it has 1 or more vms with desired roles
